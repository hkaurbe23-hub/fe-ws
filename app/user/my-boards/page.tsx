...
// ✅ FIX 1: corrected path (ProfileDropdown is in /user/, not /user/myboards/)
// ✅ FIX 2: typed the props so `email` is accepted
const ProfileDropdown = dynamic<{ email: string }>(
...
export default function UserDashboard() {
  ...
  const [boardSlaves, setBoardSlaves] = useState<Record<number, any[]>>({})
  const [slaveNames, setSlaveNames] = useState<Record<string, string>>({})
  ...
  const fetchSlaves = async (boardId: number, token: string) => {
    try {
      const res = await fetch(`${API}/api/boards/${boardId}/slaves`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await res.json()
      setBoardSlaves((prev) => ({ ...prev, [boardId]: data }))
    } catch (err) {
      console.error("Failed to fetch slaves for board", boardId, err)
    }
  }
  ...
  useEffect(() => {
    ...
    const fetchBoards = async () => {
      ...
      try {
        ...
        const data = await res.json()
        setBoards(data)
        data.forEach((b: Board) => fetchSlaves(b.id, jwtToken))
      } catch (err) {
  ...
  const handleSaveSlaveName = async (boardId: number, slaveId: number) => {
    ...
    if (jwtToken) fetchSlaves(boardId, jwtToken)
                              ...
                              {isEditing ? (
                                <>
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setSlaveNames((prev) => ({ ...prev, [key]: e.target.value }))
                                    }
                                    autoFocus
                                  />
                                  <button
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setEditingSlave(key)
                                      setSlaveNames((prev) => ({ ...prev, [key]: s.display_name || "" }))
                                    }}
                                  >
                                    <Pencil size={13} />
                                  </button>
